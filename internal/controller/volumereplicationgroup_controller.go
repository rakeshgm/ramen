...
func (v *VRGInstance) updatePVCList() error {
	...
	if v.instance.Spec.Async == nil {
		if err := v.getSyncPVCs(pvcList); err != nil {
			return fmt.Errorf("failed to get SyncPVCs (%w)", err)
		}
...
func (v *VRGInstance) getSyncPVCs(pvcList *corev1.PersistentVolumeClaimList) error {
	for idx := range pvcList.Items {
		pvc := &pvcList.Items[idx]
		...
		storageIDFromSC := storageClass.Labels[StorageIDLabel]
		storageIDsFromPeerClass := peerClass.StorageID

		storageIDFoundInPeerClass := false

		for idx := range storageIDsFromPeerClass {
			sID := storageIDsFromPeerClass[idx]
			if sID == storageIDFromSC {
				storageIDFoundInPeerClass = true
			}
		}

		if !storageIDFoundInPeerClass {
			return fmt.Errorf("storageId %s not present in peerClass", storageIDFromSC)
		}
	}
...
func (v *VRGInstance) findVolSnapClass(storageClass *storagev1.StorageClass,
	pvc *corev1.PersistentVolumeClaim) (*snapv1.VolumeSnapshotClass, error) {
