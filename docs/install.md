...
Ramen requires an [OCM](https://open-cluster-management.io/docs/concepts/architecture/) hub cluster
...
Sync mode uses a dedicated external storage cluster that all managed clusters connect to, enabling synchronous, real-time data replication.
...
- StorageClasses on managed clusters with the same `storageID` label (indicating shared storage)
- Low-latency network connectivity between managed clusters and the external storage cluster

#### Async Mode (Regional DR)

Async mode uses local storage in each managed cluster with asynchronous replication between clusters based on configurable time intervals.
...
**Required:**
- Storage provider installed in each managed cluster that supports CSI and volume replication
- StorageClasses on managed clusters with different `storageID` labels (indicating separate storage instances)
- VolumeReplication CRD and VolumeReplicationClass (for VolumeReplication-based async) OR VolSync operator and VolumeSnapshotClass (for VolSync-based async)
- Network connectivity between managed clusters for replication

For information about installing storage providers that support CSI, see your storage vendor's documentation or the [Kubernetes CSI documentation](https://kubernetes-csi.github.io/docs/).
...
The DR Cluster operator will be installed automatically during configuration. Refer to [configure.md](configure.md) for more details.
...
**Configure Ramen** - Set up DRPolicy, DRCluster resources, and S3 storage. See [configure.md](configure.md) for detailed configuration instructions.
